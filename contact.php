...
<!-- Page Content -->
    ...
    if(!empty($_GET['action']))
    {
        ...
        <h6 style="position:absolute;top:17%;left:-7%;"> choose First </h6>
        <img src="assets/images/chef4.jpeg" width="100%" height="500px" frameborder="0" style="border:0" allowfullscreen/>
        <?php } ?>
               ...
               {   
                ...
                //    ----------------------------
                 //-----------------------------------
                   ...
                   if(empty($error) && !empty($kroos))
                       { 
                       $sql="insert into content(email,name,numbers,location,product_id) VALUES (?,?,?,?,?)";
                       $stm= $conn->prepare($sql);
                       $stm->execute(array($email,$name,$subject,$Location,$kroos)); 
                       if($stm->rowCount())
                       { 
